...
export default {
  ...
  data() {
    return {
      title: "Ubah " + this.$route.params.id.split('-').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' '),
      items: [
        {
          text: "Form",
        },
        ...
        {
          text: "Ubah " + this.$route.params.id.split('-').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' '),
          active: true,
        },
      ],
      ...
      .then(result => {
        ...
        switch (this.$route.params.id) {
          case "pelaksana-pekerjaan":
            ...
            document.getElementById("input-jenis").value = result.data.type
                break
          case "pengguna":
            document.getElementById("input-email").value = result.data.email
            document.getElementById("input-username").value = result.data.username
            document.getElementById("input-peran").value = result.data.roles[0].id
          case "judul":
          case "barang":
          case "pekerjaan":
          case "area":
